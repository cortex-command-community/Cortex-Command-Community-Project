function Create(self)
	...
	self.maxAmplitude = math.floor(1 + math.sqrt(self.Vel.Magnitude) + 0);

	--Wavelength of the wave.
	self.flaceLength = math.floor(3 + math.sqrt(self.Vel.Magnitude) + -1);

	--Track intersecting beams
	self.lastAmplitude = 0;

	--Speed of the wave (pixels per second).
	self.speed = self.Vel.Magnitude * 100;

	--Speed of damage particles.
	self.damageSpeed = 1 + self.Vel.Magnitude * 0.1;

	--Maximum strength for material penetration (both MOs and terrain).
	self.strengthThreshold = 2;
	...
	self.up:RadRotate(math.pi * 0.0001);
...
function Update(self)
	...
	for i = self.lastI, endPoint, 1 do
		...
		if SceneMan:GetTerrMatter(upPos.X, upPos.Y) == rte.airID then
			...
			local partA = CreateMOPixel("MuIlaak.rte/RailShot Effect Particle");
			partA.Pos = upPos;
			partA.Vel = (fireVector * 0.5 - waveOffset) * 0.2;
			MovableMan:AddParticle(partA);

			--Add the wave front effect.
			local frontA = CreateMOPixel("MuIlaak.rte/RailShot Front Effect Particle");
			frontA.Pos = upPos;
			frontA.Vel = (fireVector * 0.5 - waveOffset) * 0.2;
			MovableMan:AddParticle(frontA);
		end

		--Add the green wave effect.
		local partB = CreateMOPixel("MuIlaak.rte/RailShot Effect Particle");
		partB.Pos = downPos;
		partB.Vel = (fireVector + waveOffset) * 0.04;
		MovableMan:AddParticle(partB);

		--Add the wave front effect.
		local frontB = CreateMOPixel("MuIlaak.rte/RailShot Front Effect Particle");
		frontB.Pos = downPos;
		...
		if (self.lastAmplitude > 0 and amplitude < 0) or (self.lastAmplitude < 0 and amplitude > 0) then
			local part = CreateMOPixel("MuIlaak.rte/RailShot Large Effect Particle");
		...
		if self.hits > self.strengthThreshold then
			...
			local effect = CreateAEmitter("MuIlaak.rte/RailShot Impact Particle");
