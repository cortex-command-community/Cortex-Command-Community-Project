...
local function emitSmoke(self, particleCount)
	...
	for i = 1, particleCount do
		local smoke = CreateMOSParticle("Plasma Flame Smoke 2 Special", "MuIlaak.rte");
		smoke.Pos = MuzzlePos;
		smoke.Lifetime = smoke.Lifetime * RangeRand(0.1, 0.3);
		smoke.Vel = var.Vel * 0.001 + Vector(RangeRand(0, i), 0):RadRotate(RangeRand(-math.pi, math.pi));
...
local function drawThiccWobblyLine(screen, startPos, endPos, thiccness, color)
	...
	local pos1 = startPos + dirVector:SetMagnitude((thiccness - 1) / 2 + math.random(-1, 1.5))
...
function Create(self)
	...
	var.range = 1000;
	var.penetrationStrength = 100;
	var.strengthVariation = 6;
...
function ThreadedUpdate(self)
	...
	if self.FiredFrame then
		...
		if rayLength >= 0 then
			...
			local pix = CreateMOPixel("Fire Burn Particle", "Browncoats.rte");
			pix.Pos = gapPos;
		...
		if rayLength ~= 0 then
			...
			for player = Activity.PLAYER_1, Activity.MAXPLAYERCOUNT - 1 do
				...
				if screen ~= -1 and not (SceneMan:IsUnseen(startPos.X, startPos.Y, team) or SceneMan:IsUnseen(hitPos.X, hitPos.Y, team)) then
					drawThiccWobblyLine(screen, startPos, startPos + trace, 3, 149);
	...
	if self.Magazine and self.Magazine.RoundCount > 0 then
		...
		emitSmoke(self, math.floor(ammoRatio * RangeRand(0, 0) + RangeRand(0.25, 0.50)));
