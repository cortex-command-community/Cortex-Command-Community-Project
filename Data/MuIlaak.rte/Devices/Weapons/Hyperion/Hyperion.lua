function Create(self)
	...
	self.activeSound = CreateSoundContainer("Hyperion Emission Sound", "MuIlaak.rte");
end
...
function ThreadedUpdate(self)
		...
		if self.Magazine.RoundCount > 0 then
			if self.animTimer:IsPastSimMS(200 * (1 - self.charge)) then
				...
				if self.Frame == 1 then
					self.effect = CreateMOPixel("Hyperion Muzzle Glow");
					self.effect.Pos = self.MuzzlePos;
					self.effect.Vel = self.Vel * 0.5;

					self.damagePar = CreateMOPixel("Hyperion Emission Particle 2");
	...
	if self.FiredFrame then
		...
		self.par = CreateAEmitter("Hyperion Shot");
