...
std::vector<PieSlice*> PieQuadrant::GetFlattenedPieSlices(bool inCCWOrder) const {
	...
	if (inCCWOrder) {
		for (auto pieSliceReverseIterator = m_RightPieSlices.crbegin(); pieSliceReverseIterator < m_RightPieSlices.crend(); pieSliceReverseIterator++) {
			if (*pieSliceReverseIterator) {
				pieSlices.emplace_back((*pieSliceReverseIterator).get());
			}
		}
		if (m_MiddlePieSlice) {
			pieSlices.emplace_back(m_MiddlePieSlice.get());
		}
		for (const std::unique_ptr<PieSlice>& pieSlice: m_LeftPieSlices) {
			if (pieSlice) {
				pieSlices.emplace_back(pieSlice.get());
			}
		}
	} else {
		if (m_MiddlePieSlice) {
		...
		for (int i = 0; i < firstSliceArray.size(); i++) {
			if (firstSliceArray[i]) {
				pieSlices.emplace_back(firstSliceArray[i].get());
			}
			if (secondSliceArray[i]) {
				pieSlices.emplace_back(secondSliceArray[i].get());
			}
		}
	}
	return pieSlices;
...
void PieQuadrant::RealignPieSlices() {
	...
	for (PieSlice* pieSlice: pieSlices) {
		...
		if (oddRoundedSliceCount < c_PieQuadrantSlotCount && pieSlice == m_MiddlePieSlice.get()) {
			sliceSlotCount = (oddRoundedSliceCount == 1) ? c_PieQuadrantSlotCount : c_PieQuadrantSlotCount - 2;
		}
