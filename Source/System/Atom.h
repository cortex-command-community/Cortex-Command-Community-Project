...
#pragma region Memory Management
		...
		/// @param fillAmount The number of instances to fill with. If 0 is specified, the set refill amount will be used.
		static void FillPool(int fillAmount = 0);

		/// Returns a raw chunk of memory back to the pre-allocated available pool.
		/// @param returnedMemory The raw chunk of memory that is being returned. Needs to be the same size as an Atom. OWNERSHIP IS TRANSFERRED!
...
#pragma region Getters and Setters
		...
		/// @return A const pointer to the owner.
...
#pragma region Concrete Methods
		...
		/// @param sprite The bitmap to check against. Ownership IS NOT transferred!
...
#pragma region Collision
		...
		/// @return The Vector describing the velocity of this Atom at the last time it hit something.
		...
		/// @return Whether or not this is ignoring all MO hits.
		...
		/// @return The ID of the non-ignored MO, if any, that this Atom is now intersecting because of the last step taken.
		...
		/// @remark Collision data should already be set by SetHitData().
		/// @return Whether this collision is valid and should be considered further.
		bool MOHitResponse();

		/// Returns the terrain material the previously taken step by TakeStep hit, if any.
		/// @return The ID of the material, if any, that this MO hit during the last step.
		...
		/// @remark Collision data should already be set by SetHitData().
		/// @return The resulting HitData of this Atom with all the information about the collision filled out.
...
#pragma region Travel
		...
		/// @return If the start pos is already intersecting the terrain or an MO, then this will return true. Do collision response as needed.
		...
		/// @return The current position of the Atom, with the offset baked in.
		...
		/// @return The total number of steps this will take in the newly set-up segment.
		int SetupSeg(Vector startPos, Vector trajectory, float stepRatio = 1.0);

		/// Gets the segment trajectory currently set by SetupSeg.
		/// @return The currently traversed segment trajectory.
		...
		/// @return A normalized float describing the step ratio.
...
#pragma region Operator Overloads
		...
		/// @param rhs An Atom reference.
		/// @return A reference to the changed Atom.
