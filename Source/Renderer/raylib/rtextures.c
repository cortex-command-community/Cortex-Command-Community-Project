...
#include "rlutils.h"              // Required for: TRACELOG()
...
//----------------------------------------------------------------------------------
#if 0 // NOTE: Disable Image functions
...
bool ExportImage(Image image, const char *fileName)
{
    ...
    {
        // NOTE: Getting RLColor array as RGBA unsigned char values
...
Image GenImageColor(int width, int height, RLColor color)
{
...
Image GenImageGradientLinear(int width, int height, int direction, RLColor start, RLColor end)
{
...
Image GenImageGradientRadial(int width, int height, float density, RLColor inner, RLColor outer)
{
    RLColor *pixels = (RLColor *)RL_MALLOC(width*height*sizeof(RLColor));
...
Image GenImageGradientSquare(int width, int height, float density, RLColor inner, RLColor outer)
{
    RLColor *pixels = (RLColor *)RL_MALLOC(width*height*sizeof(RLColor));
...
Image GenImageChecked(int width, int height, int checksX, int checksY, RLColor col1, RLColor col2)
{
    RLColor *pixels = (RLColor *)RL_MALLOC(width*height*sizeof(RLColor));
...
Image GenImagePerlinNoise(int width, int height, int offsetX, int offsetY, float scale)
{
    ...
    for (int y = 0; y < height; y++)
    {
        for (int x = 0; x < width; x++)
        {
            ...
            pixels[y*width + x] = (RLColor){ intensity, intensity, intensity, 255 };
        }
    }
...
Image ImageText(const char *text, int fontSize, RLColor color)
{
    Image imText = { 0 };
...
Image ImageTextEx(Font font, const char *text, float fontSize, float spacing, RLColor tint)
{
    Image imText = { 0 };
#if defined(SUPPORT_MODULE_RTEXT)
...
void ImageResize(Image *image, int newWidth, int newHeight)
{
    ...
    {
        // Get data as RLColor pixels array to work with it
        RLColor *pixels = LoadImageColors(*image);
        RLColor *output = (RLColor *)RL_MALLOC(newWidth*newHeight*sizeof(RLColor));

        // NOTE: RLColor data is cast to (unsigned char *), there shouldn't been any problem...
...
void ImageResizeCanvas(Image *image, int newWidth, int newHeight, int offsetX, int offsetY, RLColor fill)
{
    // Security check to avoid program crash
    if ((image->data == NULL) || (image->width == 0) || (image->height == 0)) return;
...
void ImageToPOT(Image *image, RLColor fill)
{
    // Security check to avoid program crash
    if ((image->data == NULL) || (image->width == 0) || (image->height == 0)) return;
...
void ImageAlphaClear(Image *image, RLColor color, float threshold)
{
    // Security check to avoid program crash
    if ((image->data == NULL) || (image->width == 0) || (image->height == 0)) return;
...
void ImageColorTint(Image *image, RLColor color)
{
    // Security check to avoid program crash
    if ((image->data == NULL) || (image->width == 0) || (image->height == 0)) return;
...
void ImageColorReplace(Image *image, RLColor color, RLColor replace)
{
    // Security check to avoid program crash
    if ((image->data == NULL) || (image->width == 0) || (image->height == 0)) return;
...
RLColor *LoadImageColors(Image image)
{
...
RLColor *LoadImagePalette(Image image, int maxPaletteSize, int *colorCount)
{
    #define COLOR_EQUAL(col1, col2) ((col1.r == col2.r)&&(col1.g == col2.g)&&(col1.b == col2.b)&&(col1.a == col2.a))

    int palCount = 0;
    RLColor *palette = NULL;
    RLColor *pixels = LoadImageColors(image);
...
void UnloadImagePalette(RLColor *colors)
{
    RL_FREE(colors);
}
...
RLColor GetImageColor(Image image, int x, int y)
{
    RLColor color = { 0 };
...
void ImageClearBackground(Image *dst, RLColor color)
{
    // Security check to avoid program crash
...
void ImageDrawPixel(Image *dst, int x, int y, RLColor color)
{
    // Security check to avoid program crash
...
void ImageDrawPixelV(Image *dst, Vector2 position, RLColor color)
{
    ImageDrawPixel(dst, (int)position.x, (int)position.y, color);
}

// Draw line within an image
void ImageDrawLine(Image *dst, int startPosX, int startPosY, int endPosX, int endPosY, RLColor color)
{
...
void ImageDrawLineV(Image *dst, Vector2 start, Vector2 end, RLColor color)
{
...
void ImageDrawLineEx(Image *dst, Vector2 start, Vector2 end, int thick, RLColor color)
{
    // Round start and end positions to nearest integer coordinates
    int x1 = (int)(start.x + 0.5f);
    int y1 = (int)(start.y + 0.5f);
...
void ImageDrawCircle(Image* dst, int centerX, int centerY, int radius, RLColor color)
{
    int x = 0;
    int y = radius;
...
void ImageDrawCircleV(Image* dst, Vector2 center, int radius, RLColor color)
{
    ImageDrawCircle(dst, (int)center.x, (int)center.y, radius, color);
}

// Draw circle outline within an image
void ImageDrawCircleLines(Image *dst, int centerX, int centerY, int radius, RLColor color)
{
    int x = 0;
    int y = radius;
    int decesionParameter = 3 - 2*radius;
...
void ImageDrawCircleLinesV(Image *dst, Vector2 center, int radius, RLColor color)
{
    ImageDrawCircleLines(dst, (int)center.x, (int)center.y, radius, color);
}

// Draw rectangle within an image
void ImageDrawRectangle(Image *dst, int posX, int posY, int width, int height, RLColor color)
{
    ImageDrawRectangleRec(dst, (Rectangle){ (float)posX, (float)posY, (float)width, (float)height }, color);
}

// Draw rectangle within an image (Vector version)
void ImageDrawRectangleV(Image *dst, Vector2 position, Vector2 size, RLColor color)
{
    ImageDrawRectangle(dst, (int)position.x, (int)position.y, (int)size.x, (int)size.y, color);
}

// Draw rectangle within an image
void ImageDrawRectangleRec(Image *dst, Rectangle rec, RLColor color)
{
    // Security check to avoid program crash
    if ((dst->data == NULL) || (dst->width == 0) || (dst->height == 0)) return;
...
void ImageDrawRectangleLines(Image *dst, Rectangle rec, int thick, RLColor color)
{
...
void ImageDrawTriangle(Image *dst, Vector2 v1, Vector2 v2, Vector2 v3, RLColor color)
{
...
void ImageDrawTriangleEx(Image *dst, Vector2 v1, Vector2 v2, Vector2 v3, RLColor c1, RLColor c2, RLColor c3)
{
    // Calculate the 2D bounding box of the triangle
    // Determine the minimum and maximum x and y coordinates of the triangle vertices
    int xMin = (int)((v1.x < v2.x)? ((v1.x < v3.x)? v1.x : v3.x) : ((v2.x < v3.x)? v2.x : v3.x));
...
void ImageDrawTriangleLines(Image *dst, Vector2 v1, Vector2 v2, Vector2 v3, RLColor color)
{
...
void ImageDrawTriangleFan(Image *dst, Vector2 *points, int pointCount, RLColor color)
{
    if (pointCount >= 3)
    {
...
void ImageDrawTriangleStrip(Image *dst, Vector2 *points, int pointCount, RLColor color)
{
    if (pointCount >= 3)
    {
...
// NOTE: RLColor tint is applied to source image
void ImageDraw(Image *dst, Image src, Rectangle srcRec, Rectangle dstRec, RLColor tint)
{
    // Security check to avoid program crash
...
void ImageDrawText(Image *dst, const char *text, int posX, int posY, int fontSize, RLColor color)
{
...
void ImageDrawTextEx(Image *dst, Font font, const char *text, Vector2 position, float fontSize, float spacing, RLColor tint)
{
...
// NOTE: Disable RenderTextures (already implemented)
...
#endif // NOTE: Disable lotsa stuff
...
//------------------------------------------------------------------------------------
// Draw a texture
void DrawTexture(Texture2D texture, int posX, int posY, RLColor tint)
{
    DrawTextureEx(texture, (Vector2){ (float)posX, (float)posY }, 0.0f, 1.0f, tint);
}

// Draw a texture with position defined as Vector2
void DrawTextureV(Texture2D texture, Vector2 position, RLColor tint)
{
    DrawTextureEx(texture, position, 0, 1.0f, tint);
}

// Draw a texture with extended parameters
void DrawTextureEx(Texture2D texture, Vector2 position, float rotation, float scale, RLColor tint)
{
...
void DrawTextureRec(Texture2D texture, Rectangle source, Vector2 position, RLColor tint)
{
...
void DrawTexturePro(Texture2D texture, Rectangle source, Rectangle dest, Vector2 origin, float rotation, RLColor tint)
{
...
#if 0 // NOTE: Disable NPatch
// Draws a texture (or part of it) that stretches or shrinks nicely using n-patch info
void DrawTextureNPatch(Texture2D texture, NPatchInfo nPatchInfo, Rectangle dest, Vector2 origin, float rotation, RLColor tint)
{
    if (texture.id > 0)
    {
        float width = (float)texture.width;
...
bool ColorIsEqual(RLColor col1, RLColor col2)
{
    bool result = false;
...
RLColor Fade(RLColor color, float alpha)
{
    RLColor result = color;
...
Vector4 ColorNormalize(RLColor color)
{
    Vector4 result;
...
RLColor ColorFromNormalized(Vector4 normalized)
{
    RLColor result;
...
Vector3 ColorToHSV(RLColor color)
{
    Vector3 hsv = { 0 };
...
// Get a RLColor from HSV values
// Implementation reference: https://en.wikipedia.org/wiki/HSL_and_HSV#Alternative_HSV_conversion
// NOTE: Color->HSV->RLColor conversion will not yield exactly the same color due to rounding errors
// Hue is provided in degrees: [0..360]
// Saturation/Value are provided normalized: [0.0f..1.0f]
RLColor ColorFromHSV(float hue, float saturation, float value)
{
    RLColor color = { 0, 0, 0, 255 };
...
RLColor ColorTint(RLColor color, RLColor tint)
{
    RLColor result = color;
...
RLColor ColorBrightness(RLColor color, float factor)
{
    RLColor result = color;
...
RLColor ColorContrast(RLColor color, float contrast)
{
    RLColor result = color;
...
// Get color with alpha applied, alpha goes from 0.0f to 1.0f
RLColor ColorAlpha(RLColor color, float alpha)
{
    RLColor result = color;
...
{
    RLColor out = {1.0f, 1.0f, 1.0f, 1.0f};
...
#define COLORALPHABLEND_INTEGERS
    ...
    else
    {
        ...
        out = (RLColor){ (unsigned char)(fout.x*255.0f), (unsigned char)(fout.y*255.0f), (unsigned char)(fout.z*255.0f), (unsigned char)(fout.w*255.0f) };
    }
#endif
...
RLColor ColorLerp(RLColor color1, RLColor color2, float factor)
{
    RLColor color = { 0 };
...
// Get a RLColor struct from hexadecimal value
RLColor GetColor(unsigned int hexValue)
{
    RLColor color;
...
RLColor GetPixelColor(void *srcPtr, int format)
{
    RLColor color = { 0 };

    switch (format)
    {
        case PIXELFORMAT_UNCOMPRESSED_GRAYSCALE: color = (RLColor){ ((unsigned char *)srcPtr)[0], ((unsigned char *)srcPtr)[0], ((unsigned char *)srcPtr)[0], 255 }; break;
        case PIXELFORMAT_UNCOMPRESSED_GRAY_ALPHA: color = (RLColor){ ((unsigned char *)srcPtr)[0], ((unsigned char *)srcPtr)[0], ((unsigned char *)srcPtr)[0], ((unsigned char *)srcPtr)[1] }; break;
        case PIXELFORMAT_UNCOMPRESSED_R5G6B5:
        {
        ...
        case PIXELFORMAT_UNCOMPRESSED_R8G8B8A8: color = (RLColor){ ((unsigned char *)srcPtr)[0], ((unsigned char *)srcPtr)[1], ((unsigned char *)srcPtr)[2], ((unsigned char *)srcPtr)[3] }; break;
        case PIXELFORMAT_UNCOMPRESSED_R8G8B8: color = (RLColor){ ((unsigned char *)srcPtr)[0], ((unsigned char *)srcPtr)[1], ((unsigned char *)srcPtr)[2], 255 }; break;
        case PIXELFORMAT_UNCOMPRESSED_R32:
        {
...
void SetPixelColor(void *dstPtr, RLColor color, int format)
{
    switch (format)
    {
        case PIXELFORMAT_UNCOMPRESSED_GRAYSCALE:
