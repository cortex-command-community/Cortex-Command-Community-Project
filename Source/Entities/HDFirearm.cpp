...
int HDFirearm::Save(Writer& writer) const {
	...
	writer.NewPropertyWithValue("Magazine", m_pMagazine);
	writer.NewPropertyWithValue("Flash", m_pFlash);
	writer.NewPropertyWithValue("PreFireSound", m_PreFireSound);
	writer.NewPropertyWithValue("FireSound", m_FireSound);
	writer.NewPropertyWithValue("FireEchoSound", m_FireEchoSound);
	writer.NewPropertyWithValue("ActiveSound", m_ActiveSound);
	writer.NewPropertyWithValue("DeactivationSound", m_DeactivationSound);
	writer.NewPropertyWithValue("EmptySound", m_EmptySound);
	writer.NewPropertyWithValue("ReloadStartSound", m_ReloadStartSound);
	writer.NewPropertyWithValue("ReloadEndSound", m_ReloadEndSound);
...
HashingData HDFirearm::Hash() const {
	...
	if (magazineDef) {
		uint64_t magazineHash = m_pMagazine->Hash().m_Hash;
	...
	if (preFireSoundDef) {
		uint64_t preFireSoundHash = m_PreFireSound->Hash().m_Hash;
		hashData.m_Constituents.push_back(preFireSoundHash);
		hash ^= preFireSoundHash << 2;
	}
	...
	uint64_t muzzleOffsetHash = m_MuzzleOff.Hash().m_Hash;
	hashData.m_Constituents.push_back(muzzleOffsetHash);
	hash ^= muzzleOffsetHash << 1;

	uint64_t ejectionOffsetHash = m_EjectOff.Hash().m_Hash;
	hashData.m_Constituents.push_back(ejectionOffsetHash);
	hash ^= ejectionOffsetHash << 2;
...
void HDFirearm::Activate() {
	...
	if (!IsReloading()) {
		if (m_DeactivationSound && m_DeactivationSound->IsBeingPlayed()) {
			m_DeactivationSound->FadeOut();
		}
		if (m_ActiveSound && !m_ActiveSound->IsBeingPlayed() && (m_ActiveSound->GetLoopSetting() == -1 || !wasActivated)) {
			m_ActiveSound->Play(this->m_Pos);
		}
		if (m_PreFireSound && !wasActivated && !m_PreFireSound->IsBeingPlayed()) {
			m_PreFireSound->Play(this->m_Pos);
		}
	}
}
...
void HDFirearm::Deactivate() {
	...
	if (m_FireSound && m_FireSound->GetLoopSetting() == -1) {
		m_FireSound->Stop();
	}
	if (m_DeactivationSound && wasActivated && m_FiredLastFrame) {
...
void HDFirearm::Update() {
	...
	if (m_PreFireSound && m_PreFireSound->IsBeingPlayed()) {
		m_PreFireSound->SetPosition(m_Pos);
	}
	if (m_FireSound && m_FireSound->IsBeingPlayed()) {
		m_FireSound->SetPosition(m_Pos);
	}
	if (m_ActiveSound && m_ActiveSound->IsBeingPlayed()) {
		m_ActiveSound->SetPosition(m_Pos);
	}
	if (m_DeactivationSound && m_DeactivationSound->IsBeingPlayed()) {
		m_DeactivationSound->SetPosition(m_Pos);
	}
	...
	if (m_pMagazine && !m_pMagazine->IsEmpty()) {
		if (m_Activated && !(m_PreFireSound && m_PreFireSound->IsBeingPlayed())) {
			...
			if (m_FireFrame) {
				RunScriptedFunctionInAppropriateScripts("OnFire", false, false);
			}
		} else {
	...
	if (roundsFired > 0) {
		...
		if (!m_IsAnimatedManually && m_FrameCount > 1) {
			m_Frame = 1;
		}
