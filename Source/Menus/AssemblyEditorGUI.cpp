...
bool AssemblyEditorGUI::UpdateBrainPath() {
	...
	if (m_pCurrentObject && m_pCurrentObject->IsInGroup("Brains")) {
		m_BrainSkyPathCost = g_SceneMan.GetScene()->CalculatePath(m_CursorPos, Vector(m_CursorPos.m_X, 0), m_BrainSkyPath, c_PathFindingDefaultDigStrength, FLT_MAX, static_cast<Activity::Teams>(m_pController->GetTeam()));
		return true;
	}
	...
	SceneObject* pBrain = g_SceneMan.GetScene()->GetResidentBrain(m_pController->GetPlayer());
	if (pBrain)
		m_BrainSkyPathCost = g_SceneMan.GetScene()->CalculatePath(pBrain->GetPos(), Vector(pBrain->GetPos().m_X, 0), m_BrainSkyPath, c_PathFindingDefaultDigStrength, FLT_MAX, static_cast<Activity::Teams>(m_pController->GetTeam()));
	else {
		m_BrainSkyPath.clear();
		m_BrainSkyPathCost = 0;
		return false;
